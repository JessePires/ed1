...
int retornaPosicao(Fila* f, Tipo Elemento);
void shift(Fila* f, int posicao);
...
int retornaPosicao(Fila* f, Tipo elemento){
    ...
    while(i < (f->fim % f->tam) && f->vetor[i] > elemento){
...
void shift(Fila* f, int posicao){
    ...
    for(int i = f->fim; i > posicao; i--){

        f->vetor[(i % f->tam) + 1] = f->vetor[i % f->tam];
...
Boolean fila_inserir(Fila* f, Tipo elemento){
    ...
    if(f != NULL){
        ...
        if(f->vetor[f->inicio] < elemento){

            shift(f, 0);
            f->vetor[f->inicio] = elemento;

        }else if(f->vetor[f->fim] > elemento){

            f->vetor[f->fim] = elemento;

        }else{

            int posicao = retornaPosicao(f, elemento);
            shift(f, posicao);
            f->vetor[posicao] = elemento;
